function AHfieldcorrection_Rick(root,subcode,subdir,parrecfile_short,parrecfile_long)
    magfile = [root,'/',subcode,subdir,'/B0x1.img,1'];
    phafile = [root,'/',subcode,subdir,'/B0x2.img,1'];
% convert B0 img hdr to nii
    ...
    IP.et={5.19, 6.65};
    magfile = [root,'/',subcode,subdir,'/B0mag.nii'];
    phafile = [root,'/',subcode,subdir,'/B0pha.nii'];  
    %scale phase map to radians
    new_rad = scalepha2rad(phafile);% took spms scale function
    radfile = [root,'/',subcode,subdir,'/scB0pha.nii'];
    ...
    epi = [root,'/',subcode,'/',parrecfile_short,'/0001.img'];
    IP.epiP = spm_vol(epi);
    ...
    FieldMap('Write',IP.P{1},IP.fm.fpm,'fpm_',64,'Smoothed phase map');
    [IP.vdm, IP.vdmP]=FieldMap('FM2VDM',IP);
    epi = [root,'/',subcode,'/',parrecfile_long,'/0001.img'];
    IP.epiP = spm_vol(epi);
    IP.vdmP = FieldMap('MatchVDM',IP);
